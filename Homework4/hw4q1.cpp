...
int main () {
   ...
   int done = 0; // Flag variable for end of simulation
   ...
   int actualClintonVotes = 233; // The number of electoral votes Clinton received in real life
   ...
   // Simulate the probability that she gets fewer than 233 electoral votes
   while (!done){

      // Reset the number of votes per simulation
      C = 0;
      antiC = 0;

      // Reset the indicator of the event happening
      success = 0;
      antiSuccess = 0;
      ...
      for (i = 1; i <= 51; i++){
         ...
         // Generate a fresh uniform
         U = MTUniform(0);

         // Randomly generate whether Clinton gets the votes
         if (U <= ClintonProbability[i]/100.0){
            C = C + ElectoralVotes[i];
         }

         // Antithetically generate whether Clinton gets the votes
         if (1 - U <= ClintonProbability[i]/100.0){
            antiC = antiC + ElectoralVotes[i];
         }

      }

      // Increment success and antiSuccess if the event happens
      if (C <= actualClintonVotes){ ++success; } 
